...
class OptionsMenu extends MusicBeatState
{
	...
	var leavingName="GET THE FUCK OUT OF HERE";
	public var settingsData:Array<Dynamic> = [
		//	[
		//		"Downscroll", // NAME
		//		"Makes the arrows go down", // DESC
		//		"bool", // VALUE TYPE
		//		false, // DEFAULT VALUE
		//		"Gameplay" // CATEGORY
		//		"downscroll" // SAVE NAME
		//	]
		[
			"Downscroll",
			"Makes the arrows go down",
			"bool",
			false,
			"Gameplay",
			"downscroll",
		...
		[
			"Middlescroll",
			"Makes the arrows go in the middle",
			"bool",
			false,
			"Gameplay",
			"middlescroll",
		...
		[
			"Ghost Tapping",
			"Allows you to hit keys without counting as misses",
			"bool",
			true,
			"Gameplay",
			"ghostTapping",
		...
		for (i in 0...categories.length)
			{
				...
				options.push(new OptionCatagory(catName,[]));
			};
		for (i in 0...settingsData.length) 
			{
				var setData=settingsData[i];
				if (categories.indexOf(setData[4]) >= 0)
					{
						var cat=options[categories.indexOf(setData[4])];
						trace(setData[5] + " : "  +EngineData.getOption(setData[5]));
						var option = new Option(setData);
						cat.addOption(option);				
					}
			};
	...
	override function update(elapsed:Float)
	{
		...
		FlxG.camera.followLerp = CoolUtil.sussyLerp(0.06);
		...
		else
		{
			...
			if (controls.ACCEPT)
			{
				...
				else
				{
						...
						else if(options[curSelected].getName() == leavingName)
						{
							FlxG.switchState(new MainMenuState());
						}
						...
						else
						{
							...
							for (i in 0...currentSelectedCat.getOptions().length)
								{
									var controlLabel:Alphabet = new Alphabet(0, (70 * i) + 30, currentSelectedCat.getOptions()[i].name, currentSelectedCat.getOptions()[i].boldDisplay, false);
									controlLabel.isMenuItem = true;
									controlLabel.targetY = i;
									grpControls.add(controlLabel);
									// DONT PUT X IN THE FIRST PARAMETER OF new ALPHABET() !!
									if (currentSelectedCat.getOptions()[i].type=="bool" && !currentSelectedCat.getOptions()[i].withoutCheckboxes)
										{
